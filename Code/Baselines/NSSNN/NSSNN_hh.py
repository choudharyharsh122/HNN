...
def analyH_double_well(q, p):
    return 0.5 * p**2 + 0.25 * q**4 - 0.5 * q**2

def analyH_coupled_ho(q, p):
    return p**2 + q**2 + 0.5 * q*p

def analyH_henon_heiles(q1, p1, q2, p2):
    return 0.5 * (p1**2 + p2**2) + 0.5 * (q1**2 + q2**2) + (q1**2) * q2 - (q2**3) / 3.0


# Wrapper so KAnalysis can call analyH(q, p) with q,p being shape [B,1,2]
def analyH_hh_wrapper(q, p):
    q = q.squeeze(1)  # [B,2]
    p = p.squeeze(1)
    q1 = q[..., 0]
    q2 = q[..., 1]
    p1 = p[..., 0]
    p2 = p[..., 1]
    return analyH_henon_heiles(q1, p1, q2, p2)


# === Modified initial condition sampler (matching reference) ===
def generate_initial_conditions(num_trajectories, device):
    q_init = -0.3 + 0.6 * torch.rand(num_trajectories, 2, device=device)  # [B,2]
    p_init = -0.3 + 0.6 * torch.rand(num_trajectories, 2, device=device)  # [B,2]
    # reshape to [B,1,2] to be consistent with your q0,p0 usage
    q_init = q_init.unsqueeze(1)  # [B,1,2]
    p_init = p_init.unsqueeze(1)
...
class KAnalysis(nn.Module):
    def __init__(self, analyH):
        super(KAnalysis, self).__init__()
        self.analyH = analyH  # Store the Hamiltonian function
    ...
    def forward(self, q, p):
        with torch.enable_grad():
            ...
            K = self.analyH(q, p)  # Use the stored Hamiltonian function
            K = K.sum()
...
def Gen_Data(dynamics):
    assert dynamics == 'henon_heiles', "This version of Gen_Data only supports henon_heiles"
    datau = [[] for _ in range(nsteps + 1)]
    datau_hnn = [[] for _ in range(2)]
    datat = []

    noise_level = 0.008  # uniform noise amplitude factor (gives ±0.004)

    analyH = analyH_hh_wrapper
    f_true = KAnalysis(analyH)
    f_true.eval()

    for i in range(n_sample):
        t1_t2 = torch.tensor([epsD])

        # initial conditions in [-0.3,0.3]^2
        q0 = -0.3 + 0.6 * torch.rand(1, 1, N)
        p0 = -0.3 + 0.6 * torch.rand(1, 1, N)
        ...
        with torch.no_grad():
            ...
            # add uniform noise of level 0.008 (i.e., in [-0.004, 0.004])
            q1 = q1 + noise_level * (torch.rand_like(q1) - 0.5)
            p1 = p1 + noise_level * (torch.rand_like(p1) - 0.5)

            dq0 = (q1 - q0) / t1_t2
            dp0 = (p1 - p0) / t1_t2
            datau_hnn[1].append(torch.cat([-dp0, dq0], dim=1).unsqueeze(-1))

            q0 = q1; p0 = p1; x0 = q1; y0 = p1
            datau[1].append(torch.cat([q1, p1, q1, p1], dim=1).unsqueeze(-1))

            for j in range(1, nsteps):
                q1, p1, x1, y1 = Nonsep_SymInt(q0, p0, x0, y0, t1_t2, f_true.forward, epsT)
                # add same noise for subsequent steps if desired
                q1 = q1 + noise_level * (torch.rand_like(q1) - 0.5)
                p1 = p1 + noise_level * (torch.rand_like(p1) - 0.5)
    ...
    with h5py.File(os.path.join(data_root, f"{dynamics}_data.h5"), "w") as hf:
    ...
    with h5py.File(os.path.join(data_root, f"{dynamics}_data_hnn.h5"), "w") as hf_hnn:
        hf_hnn.create_dataset('u', data=datau_hnn)

    # test trajectory (seed)
    qT = torch.tensor([[[0., 0.]]])
    pT = torch.tensor([[[-3., -3.]]])
    xT = qT; yT = pT
    ...
    torch.save(torch.cat(qpxyT), f'{dynamics}_test.dat')
    ...
    with torch.no_grad():
        for i in range(points_validation):
            qT0 = -0.3 + 0.6 * torch.rand(1, 1, N)
            pT0 = -0.3 + 0.6 * torch.rand(1, 1, N)
            xT0 = qT0; yT0 = pT0
    ...
    torch.save(torch.cat(qpxyT0), f'{dynamics}_validation0.dat')
    torch.save(torch.cat(qpxyT1), f'{dynamics}_validation1.dat')
...
class Dataset_HNN(torch.utils.data.Dataset):
    def __init__(self, data_type, dynamics):
        # Load the correct HNN data file based on the dynamics type
        f = h5py.File(f"{dynamics}_data_hnn.h5", "r")
        self.u = f['u'][:]
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data_type, dynamics):
        # Load the correct data file based on the dynamics type
        f = h5py.File(f"{dynamics}_data.h5", "r")
        self.u = f['u'][:]
        self.dt = f['dt'][:]
        split = int(self.u.shape[0] * 0.9)
        if data_type == 'train':
...
def train(model, dynamics):
    ...
    # Pass the dynamics to the Dataset for loading the correct data files
    train_data_loader = torch.utils.data.DataLoader(Dataset('train', dynamics), batch_size=512, shuffle=True)
    test_data_loader = torch.utils.data.DataLoader(Dataset('test', dynamics), batch_size=512, shuffle=True)
    ...
    for i in range(1001):
        ...
        for batch_index, data_batch in enumerate(train_data_loader):
            ...
            # Process data based on the model
        ...
        # Save model if best test loss
        if lowest_test_loss > test_loss / test_sample:
            if (model == "model_nssnn.pt"):
                torch.save(f_neur_nssnn.state_dict(), f"{dynamics}_model_nssnn.pt")
            elif (model == "model_hnn.pt"):    
                torch.save(f_neur_hnn.state_dict(), f"{dynamics}_model_hnn.pt")
            elif (model == "model_baseline.pt"):
                torch.save(f_neur_baseline.state_dict(), f"{dynamics}_model_baseline.pt")
            lowest_test_loss = test_loss / test_sample
        
        # Logging the loss data
        print(model)
    ...
    with open(f"{dynamics}_loss.dat", 'w+') as f:
...
# === Adjusted validation for Henon-Heiles ===
def validation(model, dynamics):
    # load appropriate model
    if model == f"{dynamics}_model_baseline.pt":
        f_neur_baseline.load_state_dict(torch.load(model, map_location=lambda s, l: s))
        f_neur_baseline.to(device); f_neur_baseline.eval()
    elif model == f"{dynamics}_model_hnn.pt":
        f_neur_hnn.load_state_dict(torch.load(model, map_location=lambda s, l: s))
        f_neur_hnn.to(device); f_neur_hnn.eval()
    elif model == f"{dynamics}_model_nssnn.pt":
        f_neur_nssnn.load_state_dict(torch.load(model, map_location=lambda s, l: s))
        f_neur_nssnn.to(device); f_neur_nssnn.eval()

    # analytic Hamiltonian
    if dynamics == 'henon_heiles':
        analyH = analyH_hh_wrapper
    elif dynamics == 'double_well':
        analyH = analyH_double_well
    elif dynamics == 'coupled_ho':
        analyH = analyH_coupled_ho
    else:
        raise ValueError(dynamics)

    qpxyT0 = torch.load(f'{dynamics}_validation0.dat')
    qpxyT1 = torch.load(f'{dynamics}_validation1.dat')
    Geo = 0.0
    Err = 0.0
    ...
    with torch.no_grad():
        for i in range(points_validation):
            ...
            if model == f"{dynamics}_model_nssnn.pt":
            ...
            elif model == f"{dynamics}_model_baseline.pt":
                qN1, pN1 = RK2(qT0, pT0, dtp_validation.to(device), f_neur_baseline.forward, epsN)

            Geotemp = torch.abs((analyH(qN1, pN1) - analyH(qT0, pT0)) / torch.sqrt(analyH(qT0, pT0)**2 + 1e-7)).sum().cpu().item()
            Errtemp = (torch.abs(qN1 - qT1) + torch.abs(pN1 - pT1)).sum().cpu().item()
            Geo += Geotemp
            Err += Errtemp

    print(model, Geo / points_validation, Err / points_validation)
    with open(model + 'Hamiltonian.dat', 'w') as f:
        f.write(f"{Geo/points_validation}\t\t{Err/points_validation}\n")
        
def Gen_Data_plot(dynamics):
    qT = torch.tensor([[[0.]]])
    pT = torch.tensor([[[-3.]]])
    xT = qT
    yT = pT
    ...
    with torch.no_grad():
        for i in range(plot_Np):
            ...
            qpxyT.append(torch.cat([qT, pT, xT, yT], dim=1))
    torch.save(torch.cat(qpxyT), f'{dynamics}_test_plot.dat')
...
if __name__ == '__main__':
    # Setup argparse to parse the command-line arguments
    parser = argparse.ArgumentParser(description="Run training and validation with different dynamics types")
    
    # Add argument for dynamics_type
    parser.add_argument(
        '--dynamics_name', 
        type=str, 
        choices=['double_well', 'coupled_ho', 'henon_heiles'],  # Restrict input to these values
        required=True,  # Make this argument mandatory
        help="Specify the dynamics type (double_well, coupled_ho, henon_heiles)"
    )

    # Parse the arguments
    args = parser.parse_args()

    # Pass the dynamics_name argument into Gen_Data function
    dynamics_name = args.dynamics_name

    # Start the process
    start_time = time.time()

    # Call Gen_Data with the dynamics_name argument
    Gen_Data(dynamics_name)

    # Optionally, you can call train and validation functions
    # train("model_nssnn.pt")  # Uncomment if needed
    train("model_nssnn.pt", dynamics_name)
    validation(f"{dynamics_name}_model_nssnn.pt", dynamics_name)
    Gen_Data_plot(dynamics_name)
    validation_plot(f"{dynamics_name}_model_nssnn.pt")
