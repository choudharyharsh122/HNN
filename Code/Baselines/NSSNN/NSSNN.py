...
def Gen_Data(dynamics):
    ...
    for i in range(n_sample):
        ...
        q0 = 4.*(torch.rand(1, 1, N)-0.5)
        p0 = 4.*(torch.rand(1, 1, N)-0.5)
    ...
    hf = h5py.File(os.path.join(data_root, f"{dynamics}_data.h5"), "w")
    ...
    hf_hnn = h5py.File(os.path.join(data_root_hnn, f"{dynamics}_data_hnn.h5"), "w")
    ...
    with torch.no_grad():
        for i in range(points_validation):
            qT0 = 4.*(torch.rand(1, 1, N)-0.5)
            pT0 = 4.*(torch.rand(1, 1, N)-0.5)
            xT0 = qT0
            yT0 = pT0
