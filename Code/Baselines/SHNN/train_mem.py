...
def load_data(args):
    ...
    print("here's the data path : ",data_path)
...
def train(model, data, args):
    ...
    # Save memory/time profile
    os.makedirs("profile_logs", exist_ok=True)
    with open("profile_logs/train_profile.txt", "w") as f:
        f.write(f"Runtime (s): {end_time - start_time:.2f}\n")
        f.write(f"Peak Memory (MB): {peak_memory / (1024 ** 2):.2f}\n")
...
def train_main(args):
    ...
    #model_save_path = os.path.join("models", f"model_0_{args.name}_{args.noise}_im.pt")
