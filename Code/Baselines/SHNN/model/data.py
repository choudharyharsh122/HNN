...
def choose_data(name):
    datasets = {'spring': HarmonicOscillator,
                ...
                'twobody': TwoBody,
                'coupled_ho' : CoupledHO,
                'double_well' : DoubleWellSystem,
                'henon_heiles' : HenonHeilesSystem
                }
...
class DoubleWellSystem(HamiltonianDataSet):
    """ Implements the double-well potential system. """
    ...
    def get_trajectory(self, t_span=(0, 3), rtol=1e-9, y0=None, t_eval=None, **kwargs):
        y, t = super().get_trajectory(t_span=t_span, rtol=rtol, y0=y0, t_eval=t_eval, **kwargs)
        if self.noise > 0:
            y += np.random.randn(*y.shape) * self.noise
        return y, t


class CoupledHO(HamiltonianDataSet):
    """ Implements the non-separable coupled harmonic oscillator system. """
    ...
    @classmethod
    def hamiltonian(cls, p, q, t=None):
        alpha = 0.5  # fixed for now, can be made dynamic later
        H = 0.5 * p**2 + 0.5 * q**2 + alpha * p * q
        return H
...
class HenonHeilesSystem(HamiltonianDataSet):
    """ Implements the Henon-Heiles Hamiltonian system. """
    ...
    def hamiltonian(cls, p, q, t=None):
        ...
        potential = 0.5 * (q1**2 + q2**2) + q1**2 * q2 - (q2**3) / 3.0
        return kinetic + potential

    @staticmethod
    def phase_space_boundaries():
        return -1.0, 1.0  # Adjust if needed based on energy range

    @staticmethod
    def static_initial_value():
        return np.array([-0.1, 0.2, 0.05, -0.05])  # [q1, q2, p1, p2]

    def get_trajectory(self, t_span=(0, 10), rtol=1e-9, y0=None, t_eval=None, **kwargs):
        y, t = super().get_trajectory(t_span=t_span, rtol=rtol, y0=y0, t_eval=t_eval, **kwargs)
        if self.noise > 0:
            y += np.random.randn(*y.shape) * self.noise
        return y, t
